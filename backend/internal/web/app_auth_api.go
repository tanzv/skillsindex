...
func (a *App) handleAPIAuthCSRF(w http.ResponseWriter, r *http.Request) {
	...
	if strings.TrimSpace(token) == "" {
		writeAPIError(w, r, http.StatusInternalServerError, "csrf_token_failed", a.apiMessage(r, "api.auth.csrf_issue_failed", "Failed to issue CSRF token"))
		return
	}
	writeJSON(w, http.StatusOK, map[string]any{
		"csrf_token": token,
...
func (a *App) handleAPIAuthLogin(w http.ResponseWriter, r *http.Request) {
	if a.authService == nil {
		writeAPIError(w, r, http.StatusServiceUnavailable, "service_unavailable", a.apiMessage(r, "api.auth.service_unavailable", "Authentication service unavailable"))
		return
	}

	var payload apiAuthLoginRequest
	if err := decodeJSONOrForm(r, &payload); err != nil {
		writeAPIError(w, r, http.StatusBadRequest, "invalid_payload", a.apiMessage(r, "api.auth.invalid_payload", "Invalid request payload"))
		return
	}
	...
	if username == "" || password == "" {
		writeAPIError(w, r, http.StatusBadRequest, "invalid_payload", a.apiMessage(r, "api.auth.username_password_required", "Username and password are required"))
		return
	}

	issuedIP := clientIPFromRequest(r)
	if a.loginThrottleState().limited(username, issuedIP) {
		writeAPIError(w, r, http.StatusTooManyRequests, "too_many_requests", a.apiMessage(r, "api.auth.too_many_requests", "Too many failed sign-in attempts. Try again later."))
		return
	}
	...
	if err != nil {
		...
		writeAPIError(w, r, http.StatusUnauthorized, "unauthorized", a.apiMessage(r, "api.auth.invalid_credentials", "Invalid username or password"))
		return
	}
	a.loginThrottleState().recordSuccess(username, issuedIP)
	if err := a.startUserSession(w, r, user.ID); err != nil {
		writeAPIError(w, r, http.StatusInternalServerError, "session_start_failed", a.apiMessage(r, "api.auth.session_start_failed", "Failed to start session"))
		return
	}
...
func (a *App) handleAPIAuthMe(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		writeAPIError(w, r, http.StatusInternalServerError, "settings_query_failed", a.apiMessage(r, "api.auth.settings_query_failed", "Failed to load access settings"))
		return
	}
	...
	if a.authService != nil {
		...
		if err != nil {
			writeAPIError(w, r, http.StatusNotFound, "user_not_found", a.apiMessage(r, "api.auth.user_not_found", "User not found"))
			return
		}
		user = loaded
	}
