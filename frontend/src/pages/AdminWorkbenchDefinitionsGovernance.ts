...
export const adminWorkbenchGovernanceDefinitions: Record<AdminGovernanceRoute, WorkbenchDefinition> = {
  ...
  "/admin/access": {
    ...
    subtitle: "Registration policy and account-level administrative controls.",
    resources: [
      {
        key: "registration",
    ...
    actions: [
      ...
      },
      {
        ...
        buildPath: (values) => {
          ...
          return `/api/v1/admin/accounts/${userID}/status`;
        },
        buildPayload: (values) => ({ status: String(values.status || "") }),
      ...
      },
      {
        ...
        buildPath: (values) => {
          ...
          return `/api/v1/admin/accounts/${userID}/force-signout`;
        },
        refreshResources: ["accounts"]
      },
      {
        ...
        fields: [
          { key: "user_id", label: "User ID", type: "number", required: true, min: 1 },
          { key: "new_password", label: "New Password", type: "password", required: true, placeholder: "Minimum 8 chars" }
        ],
        buildPath: (values) => {
          const userID = requiredID(values.user_id);
          if (!userID) {
          ...
          return `/api/v1/admin/accounts/${userID}/password-reset`;
        },
        buildPayload: (values) => ({ new_password: String(values.new_password || "") }),
        refreshResources: ["accounts"]
      }
    ]
  },
