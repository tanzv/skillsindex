...
describe("PublicSkillDetailPage.helpers", () => {
  ...
  it("uses interaction stats for summary rating and keeps unavailable metrics explicit", () => {
    ...
    expect(viewModel.metadataLines).toHaveLength(3);
    expect(viewModel.metadataLines.join(" ")).not.toContain("Released");
    expect(viewModel.qualityMetrics[0]?.value).toBe("9.0");
    expect(viewModel.summaryMetrics).toHaveLength(3);
    expect(viewModel.qualityMetrics[1]?.value).toBe("N/A");
    expect(viewModel.qualityMetrics[2]?.value).toBe("N/A");
  });

  it("keeps live detail rendering aligned to backend fields", () => {
    const viewModel = buildSkillDetailViewModel(
      {
        id: 11,
    ...
    expect(viewModel.titleName).toBe("Paper to Production");
    expect(viewModel.fileEntries).toEqual([{ name: "SKILL.md", displayName: "📄 SKILL.md", size: "0.1KB" }]);
    expect(viewModel.presetPreviewContent.readme).toContain("Paper to Production");
    expect(viewModel.summaryChips).toHaveLength(0);
    expect(viewModel.metadataHeading).toBe("github.com/example/paper-to-production");
    expect(viewModel.governanceState).toBe("");
