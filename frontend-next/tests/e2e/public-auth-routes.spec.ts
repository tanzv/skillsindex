...
test("renders the login route with the prototype-aligned auth layout", async ({ page }) => {
  ...
  await expect(page.getByText("Choose the sign-in method assigned to your account.")).toBeVisible();
  await expect(page.getByText("Workspace password", { exact: true })).toBeVisible();
...
test("renders workspace actions instead of sign-in actions for authenticated viewers on public skill pages", async ({ page }) => {
  ...
  const topbarActions = page.locator(".skill-detail-topbar-actions");

  await expect(topbarActions.getByRole("link", { name: "Workspace", exact: true })).toBeVisible();
  await expect(topbarActions.getByRole("link", { name: "Sign In", exact: true })).toHaveCount(0);
});
...
test("returns authenticated viewers to the current public skill page after signing in", async ({ page }) => {
  ...
  await topbarActions.getByRole("link", { name: "Sign In", exact: true }).click();
