...
test("executes recovery, release, and change approval record actions", async ({ page }) => {
  ...
  await createPane.getByRole("button", { name: "Save Record" }).click();
  ...
  await page.goto("/admin/ops/releases");
  await page.getByRole("button", { name: /Record Entry/i }).click();
  await createPane.getByLabel("Release version").fill(`v9.${suffix}`);
  await createPane.getByLabel("Release environment").selectOption("staging");
  await createPane.getByLabel("Release change ticket").fill(`CHG-${suffix}`);
  await createPane.getByLabel("Release status").selectOption("success");
  await createPane.getByLabel("Release note").fill(`Release note ${suffix}`);
  await createPane.getByRole("button", { name: "Save Record" }).click();
  await expect(page.getByText("Operations record saved.")).toBeVisible();
  ...
  await page.goto("/admin/ops/change-approvals");
  await page.getByRole("button", { name: /Record Entry/i }).click();
  await createPane.getByLabel("Change approval ticket ID").fill(`APP-${suffix}`);
  await createPane.getByLabel("Change approval reviewer").fill("ops-reviewer");
  await createPane.getByLabel("Change approval status").selectOption("approved");
  await createPane.getByLabel("Change approval note").fill(`Approval note ${suffix}`);
  await createPane.getByRole("button", { name: "Save Record" }).click();
  await expect(page.getByText("Operations record saved.")).toBeVisible();
...
test("executes backup plan and backup run record actions", async ({ page }) => {
  ...
  await createPane.getByLabel("Backup retention days").fill("45");
  await createPane.getByLabel("Backup plan enabled").check();
  await createPane.getByLabel("Backup plan note").fill(`Plan note ${suffix}`);
  await createPane.getByRole("button", { name: "Save Record" }).click();
  await expect(page.getByText("Operations record saved.")).toBeVisible();
  ...
  await page.goto("/admin/ops/backup/runs");
  await page.getByRole("button", { name: /Record Entry/i }).click();
  await createPane.getByLabel("Backup run plan key").fill(`archive-${suffix}`);
  await createPane.getByLabel("Backup run status").selectOption("success");
  await createPane.getByLabel("Backup run size MB").fill("512");
  await createPane.getByLabel("Backup run duration minutes").fill("18");
  await createPane.getByLabel("Backup run note").fill(`Run note ${suffix}`);
  await createPane.getByRole("button", { name: "Save Record" }).click();
  await expect(page.getByText("Operations record saved.")).toBeVisible();
