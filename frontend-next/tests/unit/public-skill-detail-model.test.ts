...
describe("public skill detail model", () => {
  it("builds summary, resource insights, and related skills from the API payload", () => {
    const fallback = buildPublicSkillDetailFallback(101);
    const apiRelatedSkill = {
      ...fallback.detail.skill,
      id: 301,
      name: "API Related Skill",
      description: "Real related skill from API payload.",
    };
    const detail = {
      ...fallback.detail,
    ...
    expect(model.installationSteps[0]?.label).toBe(
      messages.skillDetailInstallLabel,
    );
    expect(model.resourceInsights[0]?.label).toBe(
      messages.skillDetailResourceRepositoryLabel,
    );
    expect(model.versionHighlights.length).toBeGreaterThan(0);
    expect(model.relatedSkills).toEqual([
      expect.objectContaining({
        id: apiRelatedSkill.id,
        name: apiRelatedSkill.name,
      }),
    ]);
  });
});
